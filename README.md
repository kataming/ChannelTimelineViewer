...
- **一覧のキャッシュ**：一度取得したチャンネルの一覧は端末内に保存し、次回は**待たずに表示**。
  裏で**新着だけ**を取りに行く（既知の動画に当たった時点で取得を打ち切るので、通常は1ページ＝quota 1）
- **続きから見る**：チャンネルごとに最後に開いた動画（`lastOpenedVideoId` / `lastOpenedAt`）を保存
- **前回の停止位置から再生**：動画ごとの再生位置を端末内に保存し、次に開いたとき続きから再生（既定オン）。
  再生画面の「続きから再生する」トグルでオフにでき、「最初から」ボタンで頭出しもできる
- **終了したら次を自動再生（既定オン・いつでもオフ可）**：再生画面に常時表示されるトグルを
  **ユーザーが自分でオンにしたときだけ**、一覧の次の動画へ続けて再生する。
  既定のオフのままなら終了時に停止し、「次の動画を再生」ボタンを表示する
- **リピート（既定オフ）**：「1本」＝同じ動画を繰り返す／「全体」＝一覧の最後まで行ったら先頭に戻る
- **未視聴のみ再生（既定オフ）**：自動再生で進むとき、視聴済みを飛ばして未視聴だけを再生する
- **スキップ**：視聴済みとは別の「見るつもりがない」印（一覧ではオレンジのチェック）。
  自動再生で飛ばし、「次に見る」の候補からも外す
...
共有先の**並び順は iOS が決めるため、アプリ側から上位に指定することはできません**（Siri の使用頻度学習で決まります）。
次の手順で「よく使う項目」に固定すると、常に上位に表示されます。

1. 共有ボタン → アプリのアイコンが並ぶ行を**右端までスクロール** → **「その他」**
2. 右上の **「編集」**
3. 「Channel Timeline Viewer」の左の **「＋」** で **「よく使う項目」** に追加
4. ハンドルをドラッグして上位へ並べ替え → **「完了」**

### ⚠️ なぜ「共有 → 自動でアプリが開く」ではないのか

**iOS の仕様で、共有シート拡張から含有アプリを直接起動することはできません**
（`NSExtensionContext.open` が使えるのは Today ウィジェットと iMessage 拡張のみ。
共有拡張・アクション拡張では必ず失敗します。回避策として知られる方法は非公開 API を使うため採用しません）。

そのため本アプリは、Apple が想定している **ローカル通知**を使ってワンタップ起動を実現し、
通知を許可していない場合はクリップボード経由にフォールバックします。

| 通知の許可 | 共有した後の動き |
| --- | --- |
| 許可済み | 共有直後に**通知**が出る → **タップでアプリが開き**、そのチャンネルを表示 |
| 未許可 | URL を**クリップボード**に置く → アプリを開くと **「共有されたURLを開く」** が出る → タップで表示 |

- 通知は**共有した直後のこの1件だけ**です。お知らせ・宣伝の通知は送らず、
  サーバーからのプッシュ通知も使いません（ローカル通知のみ）。
- 許可を尋ねるのは、**共有機能を一度使ったあと**（入力画面の案内）と「ⓘ」画面からだけです。
- クリップボードは「URL が入っているか」の判定だけを行い、**中身はボタンを押すまで読みません**。

参考: [Apple Developer Forums](https://developer.apple.com/forums/thread/776488)
...
- 動画は **必ず公式の IFrame Player（WKWebView 埋め込み）** で再生します。独自プレイヤーや
  ダウンロードは行いません。
- **自動再生は既定オン**です（いつでもオフにできます）。オフのときは再生終了時に停止し、「次の動画を再生」ボタンを表示します。
  再生画面に常時表示される「終了したら次を自動再生」トグルを**ユーザーが明示的にオンにしたときだけ**、
  続けて再生します（オン状態は端末内に保存されます）。
- 自動再生をオンにした場合でも、進む先は**ユーザーが開いているチャンネル一覧の次の動画だけ**です。
  YouTube の関連動画・おすすめへ進むことはありません（`rel=0`）。
- 再生中だけ**画面の自動ロックを止めます**（置いたまま見ていて消えるのを防ぐため）。
  一時停止時・再生画面を離れたときは通常どおり自動ロックします。
- **バックグラウンド再生は行いません**。アプリを閉じる・画面をロックすると再生も止まるため、
  自動再生が裏で走り続けることはありません。
...
- 上記の上限により、**5,000本を超えるチャンネルは 5,000本までしか取得しません**（例: NASA 公式チャンネル）。
  この場合の一覧の「全N本」は取得できた範囲の本数で、チャンネルの実際の総数ではありません。
...
- [`docs/monetization-plan.md`](docs/monetization-plan.md) — **収益化方針**（アプリ内課金・サブスク・広告は採用せず、**本体価格のみ**。初回無料 → $4.99 → $9.99）
