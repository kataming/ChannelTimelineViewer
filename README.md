...
4. Channel Timeline Viewer を開くと、最初の画面に **「共有されたURLを開く」** が出るのでタップ
   → 共有されたURLを解析して動画一覧（古い順）を開く
...
1. 拡張は受け取ったURLを**クリップボードに置く**（ネットワーク通信はしません）
2. アプリを開くと、最初の画面に **「共有されたURLを開く」** が出る（クリップボードに
   URL があるかどうかの判定だけを行い、**中身はボタンを押すまで読みません**）
3. タップすると、そのチャンネルの動画一覧を開く
