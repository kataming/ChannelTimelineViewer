...
- **終了したら次を自動再生**：一覧の次の動画へ続けて再生（既定オン）。トグルでオフにすると
  従来どおり「次の動画を再生」ボタンを表示する
...
- 動画は **必ず公式の IFrame Player（WKWebView 埋め込み）** で再生します。独自プレイヤーや
  ダウンロードは行いません。
- 再生終了時は、**ユーザーが開いているチャンネル一覧の「次の動画」**へ続けて再生します（既定オン）。
  対象はその一覧の中だけで、YouTube の関連動画・おすすめへ勝手に進むことはありません（`rel=0`）。
  再生画面の**「終了したら次を自動再生」トグルでいつでもオフ**にでき、オフのときは
  従来どおり「次の動画を再生」ボタンでユーザー操作を挟みます。
