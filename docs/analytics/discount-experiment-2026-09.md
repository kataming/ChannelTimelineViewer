...
## 関連（どちらも 2026-09-21 に完了済み）

- **GA4 のカスタムディメンション**: `stage` と `result` を登録済み。登録以降のデータだけ分解できる。
- **Play RTDN**: `cloud/play-rtdn/` を `channel-timeline` へデプロイ済み（関数 `ctv-play-rtdn`）。
  Analytics とは別経路で購入の発生を確かめられる。
