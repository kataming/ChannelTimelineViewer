...
## リリース状況（2026-09-17 時点・両OSとも出し終えた）

| | 版 | 状態 |
| --- | --- | --- |
| iOS | **1.2.0 (33)** | 審査待ち（WAITING_FOR_REVIEW・2026-09-17 04:16Z 提出） |
| Android | **1.10 (14)** | 製品版に公開済み（completed） |

- iOS のビルド 32 = チャンネル追加の案内（ユーザー確認済み）、33 = 自動再生の見出し修正＋既定オン。
- Android 1.10 は `a698b25`（Watch Queue を外してビルド33相当へ戻した版）から作られている。
- Play の製品版のリリースノートは、**7言語とも本ドキュメントで用意した 1.10 の文言**が出ていることを
  `play-metadata.yml -f mode=status` で確認済み（`status` がリリースノート本文を出すようにした）。
...
## 審査提出（2026-09-17・提出済み）

iOS **1.2.0（ビルド33）** を審査へ提出済み。

- 状態 **WAITING_FOR_REVIEW** / 提出 2026-09-17T04:16:46Z（JST 13:16）
- 紐づくビルドは **33**（実機確認したもの）

⚠️ **提出の有無は「バージョンの状態」で見ること**（2026-09-17 に一度読み違えた）。

| 見えるもの | 意味 |
| --- | --- |
| `READY_FOR_REVIEW` | 提出の束はできているが、**まだ Apple へ送っていない** |
| `WAITING_FOR_REVIEW` | 送信済み。審査待ちの列に入っている |
| `IN_REVIEW` | 審査中 |

このとき `READY_FOR_REVIEW` を「取り下げられた」と誤って読んだ。実際は
**最後の送信ボタンが押されていなかっただけ**だった。提出束の `submittedDate` が空なら未送信、
と見るのが確実（`--mode diagnose` が提出の中身と各版のビルド番号まで出すようにしてある）。

**iOS は `asc_appstore_metadata.py --mode submit` で Claude 側から送信できる**
（2026-08-24 のユーザー判断）。手で押すと押し忘れが起きるので、迷ったら Claude に任せてよい。
