...
## ユーザーに推奨する確認（任意・次の作業を止めるものではない）

iPhone の TestFlight で **1.2.0 (33)** に更新し、次の3点を見ていただくことを**推奨**する。
アプリは削除せず、上書き更新でよい（既存ユーザーの設定が保たれるかの確認にもなる）。

1. 再生画面の見出しが **「自動再生」**（状態を含まない）になっている
2. スイッチを切り替えると説明文が入れ替わる
3. 「ⓘ このアプリについて」の自動再生の説明が新しい文になっている

⚠️ その iPhone で過去に一度でもトグルを触っていると、更新後もその設定のまま。
**オフのままでも不具合ではなく仕様どおり**（その場合は一度オンにして見た目だけ確認する）。
