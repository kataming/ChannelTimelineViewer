...
**App Store Connect → App → 収益化 → App 内課金 → ＋**

| 項目 | 値 |
| --- | --- |
| タイプ | **非消費型（Non-Consumable）** |
| 参照名 | `Channel Timeline Viewer Pro` |
| 製品ID | **`pro_unlock`**（作成後は変更できない。アプリのコードと一致させる） |
| 価格 | 初期は **$4.99 相当**（ローンチ価格）。将来 $9.99 相当へ変更する前提 |
| 表示名（各言語） | `Channel Timeline Viewer Pro` |
| 説明（各言語） | 買い切りで複数チャンネル保存を解放します。サブスクリプションではありません。 |
| 審査用スクリーンショット | Pro 画面（購入ボタンが見えるもの）を1枚 |
| 審査メモ | 「複数チャンネル保存を解放する非消費型の買い切りです。サブスクではありません。」 |

- **App 内課金は初回リリースと一緒に審査に出す**（アプリのバージョンに紐づけて提出する）。
  紐づけ忘れると、アプリだけ承認されて課金が使えない状態になる。
- 価格を変えてもアプリの更新は不要（価格はコードに持っていないため）。
