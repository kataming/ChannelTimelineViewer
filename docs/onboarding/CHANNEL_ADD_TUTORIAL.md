...
## 12. ⚠️ 自動テスト・スクリーンショット撮影との相性

**案内は初回起動で自動的に前に出る。** 自動テストやスクリーンショット撮影も
まっさらな状態から始まるので、**何もしないと案内が画面を塞ぐ**。
実際に iOS のスクリーンショット撮影が7言語すべて失敗した
（`Failed: Not hittable: TextField` ＝ URL 入力欄が案内に隠れていた）。

出さないようにする方法:

| | 方法 |
| --- | --- |
| iOS | 起動引数 `-channelTutorialCompleted YES`。`UserDefaults` がその値を返すので「見終わった」状態で始まる |
| Android | `--ez skipTutorial true`（**デバッグビルドのみ**）。完了フラグは変えないので利用者の状態と混ざらない |

⚠️ iOS の引数を定数にして UITests から参照することは**できない**
（UITests のターゲットからアプリ本体の型が見えない）。テスト側には文字列を直接書く。

新しく UI テストや撮影を足すときは、**必ずこれを付ける**こと。
