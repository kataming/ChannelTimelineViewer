...
- [ ] 再生画面に「自動再生」トグルが**常時表示**され、**初期状態はオン**
      （見出しに「オン」「オフ」と状態は書かない。スイッチと説明文が状態を表す）
...
> ⚠️ iOS の仕様で**共有シートからアプリは直接起動できない**（Today / iMessage 拡張のみ可）。
> 「共有 → アプリを開く → 『共有されたURLを開く』をタップ」が正しい流れ。

- [ ] **YouTube アプリ**でチャンネルのページを開く → 共有 → 一覧に **Channel Timeline Viewer** が出る
      - 出ない場合: 共有先を右端までスクロール → **「その他」** → Channel Timeline Viewer を選ぶ
- [ ] 共有シートを**下にスクロール**したアクション一覧（「Braveで開く」等の枠）に
      **「Channel Timelineで開く」** が出る → タップでも同じ受け渡しができる
      （出ない場合は同じ一覧の末尾「アクションを編集…」から有効化・並べ替え）
- [ ] **上位固定**: 「その他」→ 右上「編集」→ 「＋」で「よく使う項目」に追加 → 並べ替え → 完了
      → 次回から共有シートの先頭付近に出る
- [ ] （通知未許可の状態で）タップすると「URL を受け取りました」と表示される（**エラー表示ではない**）
- [ ] アプリを開くと最初の画面に **「共有されたURLを開く」** が出て、タップで
      **そのチャンネルの動画一覧（古い順）**が開く
- [ ] 通知が未許可のとき、共有シートの画面に **「通知を許可して開く」** ボタンが出る
      → タップ → 許可 → **その場で通知が出て、タップするとアプリが開く**
- [ ] 一度共有経由で開いたあと、入力画面に **「共有をもっと速く」** の案内が出る
      （①通知の許可 ②共有シートの先頭に固定する手順 ③この案内を閉じる）
- [ ] 「この案内を閉じる」を押すと消え、アプリを再起動しても出てこない
- [ ] 通知を許可した状態でもう一度共有 → **通知が出る** → **通知をタップするとアプリが開き、
      そのチャンネルの一覧が表示される**（クリップボードのボタンを押さなくてよい）
- [ ] 通知は共有した直後だけで、それ以外のタイミングでは何も通知が来ない
- [ ] 「ⓘ」画面の「共有シートからワンタップで開く」に許可状態が表示される
- [ ] **YouTube アプリで動画**を開く → 共有 → 同じ流れで
      **その動画を投稿したチャンネル**の一覧が開く（動画URL → channelId 解決）
- [ ] 一度開いた後は「共有されたURLを開く」が消える（同じURLを繰り返し提示しない）
