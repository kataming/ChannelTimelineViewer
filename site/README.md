...
上限は 100ページ（5000本）で、それを超えるチャンネルは先頭ぶんだけを表示する。

> ⚠️ **このとき案内を出さないこと**（2026-09-16 ユーザー指摘で削除）。
> 以前は「アプリでは続きも扱えます」と出していたが、**事実と違っていた**。
> スマホアプリ側も同じ 50件×100ページ＝5,000本で打ち切る
> （`android/app/src/main/java/.../network/YouTubeApiClient.kt` の `MAX_PAGES`）。
> 体験版だけの制限であるかのように見せてはいけない。上限を変えるなら**両方揃えて**変える。
